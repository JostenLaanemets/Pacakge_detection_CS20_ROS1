...
def create_plane_marker(a, b, c, points):
    ...
    marker.scale.x = 0.38    # width of the plane
    marker.scale.y = 0.54    # height of the plane
...
def update_temporal_filter(new_distances, alpha=0.2):
    """
...
def callback(msg, args):
    ...
    if calibration_plane is None:
        ...
        pub_plane_marker.publish(calibration_plane)
        grid_rows = rospy.get_param("~grid_rows", 8)
        grid_cols = rospy.get_param("~grid_cols", 6)
        ...
        if prev_live_distances is not None:
            for idx, (prev_d, curr_d) in enumerate(zip(prev_live_distances, live_distances)):
                ...
                if prev_d - curr_d >= 0.002:
                    # Uncomment the following line to log detailed info.
        ...
        rospy.loginfo("Published live grid distances (smoothed).")
